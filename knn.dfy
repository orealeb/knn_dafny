...
method Init(k : int, rows : int, cols : int, trainclasses : array<int> )
...
//It also needs to specify that trainSamples && trainSamples are a newly allocated array
modifies this; 
...
ensures numRows == trainSamples.Length0 && numRows == trainSamples.Length0;
ensures numCols == trainSamples.Length1 && numCols == trainSamples.Length1;
{
  numRows := rows;
  numCols:= cols;
  trainClasses := trainclasses;
  trainSamples :=  new int[numRows,numCols];
  testSamples := new int[numRows,numCols];
  kVal := k;
}
...
requires numRows == trainSamples.Length0 && numRows == trainSamples.Length0;
...
ensures this.Valid();
//ensures  forall k : nat :: k < distances.Length0 ==> 0 <= distances[k,1].Trunc < trainClasses.Length;
{
  ...
  decreases testRowNumber - tst;
  {
    ...
    invariant testRowNumber == testSamples.Length0;
    invariant trainRowNumber == trainSamples.Length0;
    //dafny method call incompleteness
    //invariant forall k : nat :: k < tr && forall j : nat :: j < trainSamples.Length1 ==> trsample[j] == real(trainSamples[k,j]); 
    //invariant forall k : nat :: k < tst && forall j : nat :: j < testSamples.Length1 ==> tstsample[j] == real(testSamples[k,j]); 
    //invariant forall k : nat :: k < tr ==> distances[k, 0] == dist; 
     ...
     tr := tr + 1;
       ...
       assert kVal >= 1;
        ...
        i := 0;
         ...
         i := 0;
         while(i < sortedDistances.Length0)
         invariant 0 <= 1 <= sortedDistances.Length0;
         {
           // assert sortedDistances[i,1].Trunc < trainClasses.Length;
            i := i + 1;
         }
       ...
       if(kVal < sortedDistances.Length0){
         ...
         invariant forall k : nat :: k < tr ==> 0 <= distances[k,1].Trunc < trainClasses.Length;
        // invariant forall k : nat :: k < i //&& topKSortedDistances[i,1].Trunc < trainClasses.Length
          //  && 0 <= k <= topKSortedDistances[k,1].Trunc < trainClasses.Length  &&  trainClasses[topKSortedDistances[k,1].Trunc] == 1 
           // && exists k :: 0 <= k < i && distances[k,0].Trunc == topKSortedDistances[k,0].Trunc 
           // ==> voteX == old(voteX) + 1;
         //invariant forall k : nat :: k < i //&& topKSortedDistances[i,1].Trunc < trainClasses.Length 
           // && 0 <= k <= topKSortedDistances[k,1].Trunc < trainClasses.Length ==> trainClasses[topKSortedDistances[k,1].Trunc] != 1 ==> voteY == old(voteY) + 1;
         {
...
//ensures forall k : nat :: k < arr.Length0 && forall j : nat :: j < arr.Length1 ==> arr[k,j] == res[k,j]; 
{
...
{
  if (n == 0) then 0.0 else distanceTo(sample1, sample2, n-1) + ((sample1[n-1] - sample2[n-1]) * (sample1[n-1] - sample2[n-1]))
}
